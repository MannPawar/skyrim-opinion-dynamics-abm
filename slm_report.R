...
CFG <- list(
  input_csv     = "results/slm_dialogue_kb.csv",
  output_html   = "results/slm_dialogue_report.html",
  judge_fig     = "figures/fig_judge_consistency_full.png",
  scenarios     = c("Imperial", "Stormcloak"),
  commoners_n   = 8L,    # illustrative commoners per scenario
  imperial_col  = "#C0392B",
  stormcloak_col= "#2980B9",
  # Independent LLM-judge consistency (Claude judging a 120-line random sample;
  # see JUDGE_RUBRIC.md and results/judge_summary_full.csv).
  judge_stance  = 85.0,
  judge_pass    = 97.5,
  judge_n       = 120L
)
...
main <- function() {
  ...
  # Embed the LLM-judge figure as a self-contained data URI, if present.
  judge_fig_html <- ""
  if (file.exists(CFG$judge_fig)) {
    b64 <- base64_enc(readBin(CFG$judge_fig, "raw",
                              n = file.info(CFG$judge_fig)$size))
    judge_fig_html <- paste0(
      '<figure class="judgefig"><img alt="LLM-judge dialogue consistency" ',
      'src="data:image/png;base64,', b64, '">',
      '<figcaption>An independent LLM judge (Claude) scored a random sample of the ',
      'generated dialogue against the ABM-assigned stance and the campaign outcome. ',
      'See <code>JUDGE_RUBRIC.md</code>.</figcaption></figure>')
  }
  ...
  css <- sprintf('
    ...
    *{box-sizing:border-box}
    ...
    .judgefig{margin:0 0 30px;text-align:center}
    .judgefig img{max-width:680px;width:100%%;border:1px solid #e2e2e2;border-radius:10px}
    .judgefig figcaption{font-size:12.5px;color:#777;margin-top:8px;max-width:680px;
  ...
  html <- paste0(
    ...
    '<div class="method">',
    '<strong>How this works.</strong> Three layers. A stochastic agent-based model evolves a ',
    'political opinion <code>p</code> for each of 1,010 named <em>Skyrim</em> NPCs over a Civil ',
    'War campaign (<code>p&nbsp;&rarr;&nbsp;0</code> Imperial, <code>p&nbsp;&rarr;&nbsp;1</code> ',
    'Stormcloak). A per-NPC lore knowledge base (scraped from the UESP wiki) supplies each ',
    'character&rsquo;s identity and a sample of their own canonical lines to anchor voice. ',
    'The opinion and lore, not a hand-written script, are passed to a local small language ',
    'model (<code>llama3.2:3b</code> via Ollama), which voices the character in-context. The ',
    'ABM is the source of truth; the lore KB grounds identity; the SLM realizes the dialogue.',
    sprintf('<div class="stats">Curated showcase &middot; corpus: %s lines across both ',
            format(n_total, big.mark=",")),
    sprintf('campaigns &middot; %.1f%% passed the in-character validator (%s/%s) &middot; ',
            100*n_ok/n_total, format(n_ok, big.mark=","), format(n_total, big.mark=",")),
    sprintf('an independent LLM judge (Claude) scored a %d-line random sample at %.0f%% ',
            CFG$judge_n, CFG$judge_stance),
    sprintf('stance-consistent and %.0f%% shippable, with zero stance flips.</div></div>',
            CFG$judge_pass),
    judge_fig_html,
    scen_html,
    '<footer>Generated by <code>slm_report.R</code> from <code>results/slm_dialogue_kb.csv</code>. ',
    'Dialogue produced by a 3B-parameter local model; minor disfluencies are expected at this scale.</footer>',
    '</body></html>')
